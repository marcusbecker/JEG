...
public class GraphicTool {
    ...
    public boolean intersecElement(ElementModel eA, ElementModel eB) {
        ...
        return pwA >= eB.getHitX() && eA.getHitX() <= pwB && phA >= eB.getHitY() && eA.getHitY() <= phB;
    }
    ...
    public boolean collideInX(ElementModel eA, ElementModel eB) {
        if (!eA.isEnabled() || !eB.isEnabled()) {
            return false;
        }

        return eA.getPx() + eA.getWidth() >= eB.getPx() && eA.getPx() <= eB.getPx() + eB.getWidth();

    }

    public boolean collideInY(ElementModel eA, ElementModel eB) {
        if (!eA.isEnabled() || !eB.isEnabled()) {
            return false;
        }

        return eA.getPy() + eA.getHeight() >= eB.getPy() && eA.getPy() <= eB.getPy() + eB.getHeight();

    }

    public boolean inside(ElementModel eA, ElementModel eB) {
